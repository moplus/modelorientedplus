...
{
	///--------------------------------------------------------------------------------
	/// <summary>This class implements necessary members for interpretation of sort clauses.</summary>
	///
	/// <CreatedByUserName>DAVE\JavaDave</CreatedByUserName>
	/// <CreatedDate>9/29/2011</CreatedDate>
	/// <Status>Customized</Status>
	///--------------------------------------------------------------------------------
	public class SortClauseNode : NonLeafGrammarNode
	{
		///--------------------------------------------------------------------------------
		...
		public override string DisplayName
		{
			get
			{
				return LineNumber.ToString() + ": " + DisplayValues.TemplateNode_SortClause;
			}
		}

		public ModelPropertyNode ModelProperty { get; set; }
		public SortDirectionClauseNode SortDirectionClause { get; set; }
		...
		public override void Init(ParsingContext context, ParseTreeNode treeNode)
		{
			try
			{
				...
				foreach (ParseTreeNode node in treeNode.ChildNodes)
				{
					...
					else if (node.AstNode is SortDirectionClauseNode)
					{
						SortDirectionClause = node.AstNode as SortDirectionClauseNode;
						ChildNodes.Add(node.AstNode as SortDirectionClauseNode);
					}
				}
			}
