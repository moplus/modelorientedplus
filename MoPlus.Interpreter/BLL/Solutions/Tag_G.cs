...
namespace MoPlus.Interpreter.BLL.Solutions
{
	...
	/// specific Tag instances.</summary>
	///
	...
	///--------------------------------------------------------------------------------
	[Serializable()]
	[XmlRoot(Namespace="", ElementName="Tag")]
	[DataContract]
	...
	public partial class Tag : BusinessObjectBase
	{
		...
		public virtual string GetValidationErrors()
		{
			...
			error = GetValidationError("TagName");
		...
		///--------------------------------------------------------------------------------
		/// <summary>This method validates TagName and returns an error message if not valid.</param>
		///--------------------------------------------------------------------------------
		public string ValidateTagName()
		{
			if (!Regex.IsMatch(TagName, Resources.DisplayValues.Regex_Name))
			{
				return String.Format(Resources.DisplayValues.Validation_AlphanumericValue, "TagName");
			}
			return null;
		}
		...
		private Tag _reverseInstance = null;
		///--------------------------------------------------------------------------------
		/// <summary>This property gets the reverse engineering instance.</summary>
		///--------------------------------------------------------------------------------
		[XmlIgnore]
		public new Tag ReverseInstance
		{
			get
			{
		...
		public virtual string DefaultSourceName
		{
			get
			{
				...
				{
					_defaultSourceName = DefaultSourcePrefix + SourceTag.TagName;
				}
		...
		///--------------------------------------------------------------------------------
		/// <summary>This property gets/sets the OldTagID of the instance.</summary>
		///--------------------------------------------------------------------------------
		[XmlIgnore]
		...
		protected Guid _tagID = DefaultValue.Guid;
		///--------------------------------------------------------------------------------
		/// <summary>This property gets or sets the TagID.</summary>
		///--------------------------------------------------------------------------------
		[XmlElement()]
		[DataMember]
		[DataElement]
		...
		protected string _tagName = DefaultValue.String;
		///--------------------------------------------------------------------------------
		/// <summary>This property gets or sets the TagName.</summary>
		///--------------------------------------------------------------------------------
		[XmlElement()]
		[DataMember]
		[DataElement]
		...
		public override string PrimaryKeyValues
		{
			...
			set
			{
				...
				{
					TagID = primaryKeyValues[0].GetGuid();
				}
			}
		}
		...
		public override void AddTag(string tagName)
		{
			...
			{
				ForwardInstance = new Tag();
				ForwardInstance.TagID = TagID;
			}
		...
		{
			if (TagName.GetString() != inputTag.TagName.GetString()) return false;
			if (IsAutoUpdated.GetBool() != inputTag.IsAutoUpdated.GetBool()) return false;
			if (Description.GetString() != inputTag.Description.GetString()) return false;
		...
		/// <param name="inputTag">The tag to compare metadata.</param>
		///--------------------------------------------------------------------------------
		public bool IsEmptyMetadata(Tag inputTag)
		{
			if (inputTag == null) return true;
			if (inputTag.TagList.Count > 0) return false;
			if (!String.IsNullOrEmpty(inputTag.TagName)) return false;
			if (IsAutoUpdated != inputTag.IsAutoUpdated) return false;
			if (!String.IsNullOrEmpty(inputTag.Description)) return false;
		...
		///
		/// <param name="inputTag">The tag to get metadata.</param>
		///--------------------------------------------------------------------------------
		public void CopyChangedMetadata(Tag inputTag)
		{
			TagName = inputTag.TagName;
			IsAutoUpdated = inputTag.IsAutoUpdated;
		...
		public virtual void SetID()
		{
			...
			{
				TagID = Solution.UsedModelIDs[DefaultSourceName].GetGuid();
			}
			else
			{
				if (TagID == Guid.Empty)
				...
				Solution.UsedModelIDs[DefaultSourceName] = TagID;
			}
		}
		...
		///--------------------------------------------------------------------------------
		public Tag GetForwardInstance(Solution forwardSolution)
		{
			bool isCustomized = false;
			Tag forwardItem = new Tag();
			if (ForwardInstance != null)
			{
		...
		///--------------------------------------------------------------------------------
		/// <summary>This method gets the Tag instance from an xml file.</summary>
		///
		...
		///--------------------------------------------------------------------------------
		/// <summary>This method saves the Tag instance to an xml file.</summary>
		///
		...
		///--------------------------------------------------------------------------------
		public Tag(string primaryKeyValues)
		{
