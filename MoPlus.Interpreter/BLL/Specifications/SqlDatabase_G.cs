...
namespace MoPlus.Interpreter.BLL.Specifications
{
	...
	public partial class SqlDatabase : BusinessObjectBase
	{
		...
		protected EnterpriseDataObjectList<BLL.Specifications.SqlView> _sqlViewList = null;
		///--------------------------------------------------------------------------------
		/// <summary>This property gets or sets a collection of SqlDatabase.</summary>
		///--------------------------------------------------------------------------------
		[XmlIgnore]
		public virtual EnterpriseDataObjectList<BLL.Specifications.SqlView> SqlViewList
		{
			get
			{
				if (_sqlViewList == null)
				{
					_sqlViewList = new EnterpriseDataObjectList<BLL.Specifications.SqlView>();
				}
				return _sqlViewList;
			}
			set
			{
				if (_sqlViewList == null || _sqlViewList.Equals(value) == false)
				{
					_sqlViewList = value;
					if (value != null)
					{
		...
		[XmlArray(ElementName = "SqlViewList")]
		[XmlArrayItem(typeof(BLL.Specifications.SqlView), ElementName = "SqlView")]
		[DataMember(Name = "SqlViewList")]
		[DataArrayItem(ElementName = "SqlViewList")]
		public virtual EnterpriseDataObjectList<BLL.Specifications.SqlView> _S_SqlViewList
		{
			get
			{
				return _sqlViewList;
		...
		public override bool IsModified
		{
			get
			{
				...
				if (_sqlViewList != null && _sqlViewList.IsModified == true) return true;
				return false;
			}
		}
		...
		public virtual void AddItemToUsedTags(NameObjectCollection usedTags)
		{
			...
			foreach (SqlView sqlView in SqlViewList)
			{
				sqlView.AddItemToUsedTags(usedTags);
			}
		}
		...
		protected override void OnDispose()
		{
			...
			if (_sqlViewList != null)
			{
				foreach (SqlView item in SqlViewList)
				{
					item.Dispose();
				}
				SqlViewList.Clear();
		...
		public SqlDatabase GetForwardInstance(Solution forwardSolution)
		{
			...
			{
				item.SqlDatabase = this;
				SqlView forwardChildItem = item.GetForwardInstance(forwardSolution);
				if (forwardChildItem != null)
				{
					forwardItem.SqlViewList.Add(forwardChildItem);
					isCustomized = true;
				}
			}
		...
		public override void ResetLoaded(bool isLoaded)
		{
			...
			if (_sqlViewList != null) _sqlViewList.ResetLoaded(isLoaded);
		}
		...
		public override void ResetModified(bool isModified)
		{
			...
			if (_sqlViewList != null) _sqlViewList.ResetModified(isModified);
		}
