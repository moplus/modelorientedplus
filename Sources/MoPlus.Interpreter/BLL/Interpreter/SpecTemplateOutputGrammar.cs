...
{
	///--------------------------------------------------------------------------------
	/// <summary>This class defines the grammar for the spec template output.</summary>
	///
	/// <CreatedByUserName>DAVE\JavaDave</CreatedByUserName>
	/// <CreatedDate>9/8/2011</CreatedDate>
	/// <Status>Customized</Status>
	///--------------------------------------------------------------------------------
	public partial class SpecTemplateOutputGrammar : Grammar
	{
		...
		///--------------------------------------------------------------------------------
		public SpecTemplateOutputGrammar()
		{
			bool isFirstItem = true;
			this.LanguageFlags |= LanguageFlags.CreateAst;
			this.GrammarComments = "This grammar is used to parse the OUTPUT section of an M+ spec template.";
			...
			var addModelItemStatement = new NonTerminal("addModelItemStatement", typeof(AddModelItemStatementNode));
			var deleteModelItemStatement = new NonTerminal("deleteModelItemStatement", typeof(DeleteModelItemStatementNode));
			...
			var specCurrentItem = new NonTerminal("specCurrentItem", typeof(SpecCurrentItemNode));
			...
			statement.Rule = ifStatement
							...
							| paramStatement
							| assignmentStatement
							| addModelItemStatement
							| deleteModelItemStatement
							| property
							| progressStatement;
			...
			currentItemAssignmentStatement.Rule = currentItem + dot + assignableProperty + "=" + parameter
													| currentItem + "=" + modelContext
													| currentItem + "=" + "null"
													| specCurrentItem + "=" + modelContext
													| specCurrentItem + "=" + "null"
													| currentItem + "=" + "New" + Lpar + Rpar;
			...
			// add model item statement
			addModelItemStatement.Rule = ToTerm(LanguageTerms.AddTerm) + Lpar + currentItem + Rpar;

			// delete model item statement
			deleteModelItemStatement.Rule = ToTerm(LanguageTerms.DeleteTerm) + Lpar + currentItem + Rpar;
			...
			contextHelper.Rule = modelContext + dot + LanguageTerms.FindMethod + Lpar + modelProperty + comma + parameter + Rpar
								...
								| specCurrentItem + dot + LanguageTerms.FindMethod + Lpar + modelProperty + comma + parameter + Rpar
								| specCurrentItem + dot + LanguageTerms.FindMethod + Lpar + parameter + Rpar;
			...
			collectionHelper.Rule = modelContext + dot + LanguageTerms.FindAllMethod + Lpar + modelProperty + comma + parameter + Rpar
								...
								| specCurrentItem + dot + LanguageTerms.FindAllMethod + Lpar + modelProperty + comma + parameter + Rpar
			...
			// a spec current item node is a valid name of a type of node in the specification that can be assigned to
			isFirstItem = true;
			// add all SpecCurrentItemTypeCode enums to specCurrentItem rule
			foreach (string key in GrammarHelper.SpecCurrentItemTypes.AllKeys)
			{
				if (isFirstItem == true)
				{
					specCurrentItem.Rule = ToTerm(key);
					isFirstItem = false;
				}
				else
				{
					specCurrentItem.Rule = specCurrentItem.Rule | ToTerm(key);
				}
			}
			...
			// add all SpecModelContextTypeCode enums to modelContext rule
			...
			modelProperty.Rule = assignableProperty
									...
									| specCurrentItem + dot + modelProperty
									| templateProperty
			...
			// add all SpecPropertyCode enums to modelProperty rule
			...
			configurationProperty.Rule = ToTerm(LanguageTerms.UserMethod)
